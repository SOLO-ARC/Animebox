...
open class FullScreenPlayer : AbstractPlayerFragment<FragmentPlayerBinding>(
    ...
    protected fun animateLayoutChanges() {
        ...
        playerBinding?.apply {
            ...
            // Disabled horizontal slide for episodes button since it is now part of the bottom bar
    ...
    override fun onBindingCreated(binding: FragmentPlayerBinding, savedInstanceState: Bundle?) {
        ...
        playerBinding?.apply {
            ...
            playerCastCustom.setOnClickListener {
                autoHide()
                playerMediaRouteButton.performClick()
            }

            playerPipCustom.setOnClickListener {
                autoHide()
                activity?.let { act ->
                    try {
            ...
            playerMoreBtt.setOnClickListener {
                autoHide()
                playerMoreOptionsHolder.visibility = if (playerMoreOptionsHolder.visibility == View.VISIBLE) View.GONE else View.VISIBLE
            }

            try {
                val act = activity
                if (act != null) {
                    com.google.android.gms.cast.framework.CastButtonFactory.setUpMediaRouteButton(act, playerMediaRouteButton)
                }
            } catch (e: Exception) {
                e.printStackTrace()
            }
